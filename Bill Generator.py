...
c = conn.cursor()
c.execute('CREATE TABLE IF NOT EXISTS MENU (Refrence TEXT, Fries TEXT, Noodles TEXT, Soup Text, Burger TExt, Sandwich TEXT, Drinks TEXT, StateTax TEXT, ServiceCharge TEXT, SubTotal TEXT, Total TEXT)')
root=Tk()
...
lblInfo=Label(Tops,font=('helvetica',50,'bold'),text="SHRAVAN RESTAURANT ",fg="Black",bd=10,anchor='w')
...
def Ref():
    ...
    c.execute('INSERT INTO Menu (Refrence, Fries , Noodles, Soup, Burger, Sandwich, Drinks, StateTax, ServiceCharge, SubTotal, Total) VALUES(?,?,?,?,?,?,?,?,?,?,?,?)', (randomRef,CostofFries, CostOfNoodles,CostOfSoup,CostBurger, CostSandwich, CostOfDrinks, PaidTax, Service, CostOfMeal, OverAllCost))
    conn.commit()
...
btnTotal=Button(f1,padx=16,pady=8,bd=16,fg="black",font=('arial',16,'bold'),width=10,text="Total",bg="powder blue",command=Ref).grid(row=7, column=2)
